package GestionSportCol.liste_Etudiant_équipe;

import DataBase.src.Connexion;

import GestionSportCol.afficherSport.SportCol;
import GestionSportCol.liste_Equipe.Equipe;
...
public class Controller implements Initializable{
  private ObservableList <Etudiant> list ;
    ...
    public void retour(ActionEvent event) throws IOException {
        try {
            URL url = Paths.get("./Fxml/h8.fxml").toUri().toURL();
    ...
    public void load(){
        try {
            Equipe selected=  GestionSportCol.liste_Equipe.Controller.table3.getSelectionModel().getSelectedItem();
            int id= selected.getId();
            list= FXCollections.observableArrayList();

            ResultSet rs=conn.createStatement().executeQuery("SELECT * FROM `etudiant`  WHERE `etudiant`.`équipe_idéquipe` = '"+id+"'");
...
public void load2(){
    try {
        Equipe selected=  GestionSportCol.liste_Equipe.Controller.table3.getSelectionModel().getSelectedItem();
        ResultSet rs=conn.createStatement().executeQuery("SELECT * FROM `entraineur` WHERE `équipe_idéquipe` = '"+selected.getId()+"'");

        String name = rs.getString(2);
        String surname = rs.getString(3);
        nom.setText(name +" "+surname);
