package GestionSportInd.liste_Etudiant;
...
public class Controller implements Initializable {
  private ObservableList <Etudiant> list ;
    ...
    public void load(){
        ...
        try {
            SportInd selected=  GestionSportInd.afficherSport.Controller.table2.getSelectionModel().getSelectedItem();
            int id= selected.getId();
            list= FXCollections.observableArrayList();

            ResultSet rs=conn.createStatement().executeQuery("SELECT * FROM `etudiant`  WHERE `Sport_Idividuel_idSport_Idividuel` = '"+id+"'");
